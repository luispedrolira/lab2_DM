...
# 1) Variables numéricas (puedes reutilizar tu selección previa)
num_vars <- names(df_pca)[sapply(df_pca, is.numeric)]
num_vars <- intersect(num_vars, c("popularity","budget","revenue","runtime",
                                  "voteCount","voteAvg","actorsPopularity",
                                  "actorsAmount","castWomenAmount","castMenAmount",
                                  "genresAmount","productionCoAmount",
                                  "productionCountriesAmount","releaseYear"))
num_vars <- intersect(num_vars, names(df_pca))

# 2) Variables categóricas (baja cardinalidad)
cat_vars_all <- names(df_pca)[sapply(df_pca, function(x) is.character(x) || is.factor(x))]
cat_vars_all <- setdiff(cat_vars_all, c())  # si quieres excluir algo

# quedarnos con <=10 niveles para no explotar dimensiones
cat_vars <- cat_vars_all[sapply(df_pca[cat_vars_all], function(x) dplyr::n_distinct(x, na.rm = TRUE) <= 10)]
cat_vars <- intersect(cat_vars, names(df_pca))
cat_vars

# 3) Agrupar niveles raros en "Other" para estabilidad (si aplica)
collapse_rare <- function(x, min_pct = 0.02){
  x <- as.character(x)
  x[is.na(x)] <- "Missing"
  tab <- prop.table(table(x))
  rare_levels <- names(tab)[tab < min_pct]
  x[x %in% rare_levels] <- "Other"
  factor(x)
}

df_cat <- df_pca |> dplyr::select(dplyr::all_of(cat_vars)) |>
  dplyr::mutate(dplyr::across(dplyr::everything(), collapse_rare))

# 4) One-hot encoding
if(length(cat_vars) > 0){
  X_cat <- model.matrix(~ . - 1, data = df_cat) |> as.data.frame()
} else {
  X_cat <- data.frame()
}

# 5) Preparar numéricas (imputar + log1p a sesgadas + escalar)
df_num <- df_pca |> dplyr::select(dplyr::all_of(num_vars))

df_num[df_num == ""] <- NA
df_num <- df_num |> dplyr::mutate(dplyr::across(dplyr::everything(),
                                                ~ suppressWarnings(as.numeric(.x))))

impute_median <- function(x) ifelse(is.na(x), median(x, na.rm = TRUE), x)
df_num_imp <- df_num |> dplyr::mutate(dplyr::across(dplyr::everything(), impute_median))

vars_log1p <- intersect(c("budget","revenue","voteCount","popularity","actorsPopularity"), num_vars)
df_num_tr <- df_num_imp |> dplyr::mutate(dplyr::across(dplyr::all_of(vars_log1p),
                                                       ~ log1p(pmax(.x, 0))))

X_num <- scale(df_num_tr) |> as.data.frame()

# 6) Matriz final y PCA
if(ncol(X_cat) == 0){
  X_mix <- X_num
} else {
  X_mix <- cbind(X_num, X_cat)
}

pca_mix <- prcomp(X_mix, center = TRUE, scale. = FALSE)
summary(pca_mix)
fviz_eig(pca_mix, addlabels = TRUE)
...
# Usamos la misma matriz X escalada del clustering
# (ya está transformada y estandarizada)
...
# Agregar clusters de k-means para interpretación
umap_df$cluster <- factor(km_fit$cluster)
...
ggplot(umap_df, aes(x = UMAP1, y = UMAP2, color = cluster)) +
  ...
  theme_minimal() +
  labs(title = "Proyección UMAP de las películas",
       subtitle = "Coloreado por clusters k-means",
       x = "UMAP 1",
       y = "UMAP 2")
```

Justificación del uso de UMAP

Se seleccionó el algoritmo UMAP (Uniform Manifold Approximation and Projection) debido a que el conjunto de datos presenta múltiples variables numéricas con posibles relaciones no lineales. Mientras que el PCA únicamente captura relaciones lineales, UMAP permite preservar tanto estructura local como global del espacio de alta dimensión, facilitando la visualización de patrones complejos.

Dado que el dataset contiene casi 20,000 películas, UMAP resulta más eficiente y estable que alternativas como t-SNE, especialmente en términos de tiempo computacional y preservación de estructura global.

Por estas razones, UMAP es adecuado para explorar la geometría interna del dataset y validar la existencia de estructuras latentes más allá de las detectadas por clustering tradicional.


### 4.2 Interpretación de los resultados
La proyección UMAP muestra una estructura claramente no aleatoria en los datos, evidenciando regiones densas y separaciones visibles entre grupos de películas. Al colorear los puntos según los clusters obtenidos por k-means, se observa que varios grupos presentan buena separación espacial, lo cual valida la consistencia del clustering realizado previamente.

En particular, algunos clusters aparecen concentrados en zonas bien delimitadas del espacio UMAP, lo que indica alta cohesión interna y similitud entre las películas que los componen. Otros grupos muestran cierto grado de superposición, lo cual es esperado en datasets reales donde las fronteras entre segmentos no son completamente rígidas.

Un hallazgo relevante es que la estructura revelada por UMAP confirma que las películas pueden diferenciarse en múltiples dimensiones simultáneamente (impacto comercial, complejidad de producción, estructura del elenco, etc.), lo que respalda la segmentación obtenida anteriormente.

Desde una perspectiva estratégica para CineVision Studios, estos resultados sugieren que el mercado cinematográfico no es homogéneo, sino que existen nichos bien definidos. Esta visualización puede utilizarse para:

Detectar segmentos de películas similares.

Identificar posibles outliers (producciones atípicas).

Apoyar decisiones de posicionamiento y desarrollo de nuevos proyectos.

En conclusión, la aplicación de UMAP aporta valor exploratorio y confirma la existencia de estructura interna en el dataset, reforzando los hallazgos obtenidos mediante clustering y PCA.
...
# 1) Seleccionar categóricas de baja cardinalidad (<= 20 niveles)
cat_vars_all <- names(df_rules)[sapply(df_rules, function(x) is.character(x) || is.factor(x))]
cat_vars_all <- setdiff(cat_vars_all, c("title","overview","tagline","id"))  # por si existen

cat_vars <- cat_vars_all[sapply(df_rules[cat_vars_all], function(x) dplyr::n_distinct(x, na.rm = TRUE) <= 20)]

# 2) Seleccionar numéricas clave y discretizarlas (si existen)
num_candidates <- c("popularity","budget","revenue","runtime","voteCount","voteAvg","releaseYear",
                    "actorsAmount","genresAmount","productionCoAmount","productionCountriesAmount")
num_vars <- intersect(num_candidates, names(df_rules))
num_vars <- num_vars[sapply(df_rules[num_vars], is.numeric)]

# función para discretizar en cuantiles (robusto a escalas)
disc_q <- function(x, k = 4){
  x <- suppressWarnings(as.numeric(x))
  x[is.na(x)] <- median(x, na.rm = TRUE)
  arules::discretize(x, method = "frequency", categories = k, ordered = TRUE)
}

df_cat <- df_rules |> dplyr::select(dplyr::all_of(cat_vars)) |>
  dplyr::mutate(dplyr::across(dplyr::everything(), ~ as.factor(tidyr::replace_na(as.character(.x), "Missing"))))

df_num_disc <- NULL
if(length(num_vars) > 0){
  df_num_disc <- df_rules |> dplyr::select(dplyr::all_of(num_vars)) |>
    dplyr::mutate(dplyr::across(dplyr::everything(), ~ disc_q(.x, k = 4)))
}

# 3) Dataset final para transacciones
df_items <- dplyr::bind_cols(df_cat, df_num_disc)
...
```
```{r}
item_freq <- sort(arules::itemFrequency(trans), decreasing = TRUE)
head(item_freq, 20)

# Plot rápido 
arules::itemFrequencyPlot(trans, topN = 20, type = "absolute")
```

```{r}
param_grid <- list(
  list(supp = 0.05, conf = 0.6),
  list(supp = 0.03, conf = 0.6),
  list(supp = 0.02, conf = 0.65),
  list(supp = 0.01, conf = 0.7)
)
...
for(i in seq_along(param_grid)){
  ...
  r <- arules::apriori(
    trans,
    parameter = list(support = s, confidence = c, minlen = 2, maxlen = 5, target = "rules")
  )
  
  # Filtrar reglas débiles (lift > 1 mejora respecto a azar)
  r <- arules::subset(r, subset = lift > 1.05)
  rules_list[[paste0("supp_", s, "_conf_", c)]] <- r
}

# Resumen de cuántas reglas por configuración
sapply(rules_list, length)
```
```{r}
# Elegir la configuración con "buen balance" (ajusta si quieres)
key_name <- names(rules_list)[which.max(sapply(rules_list, length))]
rules_best <- rules_list[[key_name]]

key_name
summary(rules_best)

# Top 15 por lift
top_lift <- sort(rules_best, by = "lift", decreasing = TRUE)[1:min(15, length(rules_best))]
inspect(top_lift)

# Top 15 por confianza
top_conf <- sort(rules_best, by = "confidence", decreasing = TRUE)[1:min(15, length(rules_best))]
inspect(top_conf)
```
```{r}
class(rules_best)

# 2) Scatterplot: soporte (x) vs confianza (y), coloreado por lift
plot(rules_best,
     method  = "scatterplot",
...
## 5. Hallazgos Integrados y Recomendaciones Estratégicas para CineVision

***Hallazgos Integrados y Recomendaciones Estratégicas para CineVision***

El análisis realizado combinó técnicas de clustering, análisis de componentes principales, reglas de asociación y un algoritmo adicional de aprendizaje no supervisado, permitiendo obtener una visión integral del comportamiento del mercado cinematográfico.

Integración de resultados
	•	El clustering permitió identificar perfiles diferenciados de películas en función de su desempeño comercial y características estructurales.
	•	El PCA reveló que las dimensiones más relevantes del dataset están asociadas principalmente al impacto comercial y a la complejidad productiva.
	•	Las reglas de asociación evidenciaron relaciones frecuentes entre variables como presupuesto, ingresos y popularidad del elenco, reforzando los hallazgos del agrupamiento.
	•	El algoritmo adicional aplicado permitió visualizar patrones latentes y confirmar la existencia de estructuras internas coherentes en los datos.

En conjunto, los resultados muestran que el desempeño financiero y la estructura del elenco son factores determinantes en la segmentación del mercado.



***Recomendaciones concretas para CineVision***
	1.	Segmentar proyectos según perfil de inversión (alto presupuesto vs producciones independientes).
	2.	Fortalecer estrategias de marketing en películas con alta valoración crítica pero bajo revenue.
	3.	Considerar la popularidad del elenco como variable clave en decisiones de producción.
	4.	Evaluar oportunidades en producciones internacionales cuando se detecten patrones asociados a mayor retorno.
	5.	Utilizar técnicas de reducción de dimensionalidad como base para futuros modelos predictivos de éxito comercial.