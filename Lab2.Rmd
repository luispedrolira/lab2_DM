...
if(! "janitor" %in% installed.packages()) install.packages("janitor", depend = TRUE)
if(! "lubridate" %in% installed.packages()) install.packages("lubridate", depend = TRUE)
if(! "skimr" %in% installed.packages()) install.packages("skimr", depend = TRUE)
if(! "clustertend" %in% installed.packages()) install.packages("clustertend", depend = TRUE)
if(! "factoextra" %in% installed.packages()) install.packages("factoextra", depend = TRUE)
...
data_path <- "movies_2026.csv"   # <- por ejemplo

# Intenta leer CSV; si usas otro formato (RDS/Excel), cambia este bloque.
...
df <- df_raw |> select(-any_of(vars_drop_if_exist))
